...
def build_practice_trial_html(row: dict) -> str:
    ...
    return "\n".join(
        [
            ...
            "  <p><strong>KO</strong> 좌우의 큰 슬라이더는 재생 구간을 설정하고, 상하 슬라이더는 점수 입력용입니다. 음성의 뒷부분은 공백인 경우가 많으니 재생 구간을 적절하게 설정하시면 빠르게 실험을 끝내실 수 있습니다.</p>",
...
def build_trial_content_html(trial_idx: int, total_trials: int, row: dict) -> str:
    ...
    return "\n".join(
        [
            ...
            "      KO: 이미지/프롬프트와의 일치도와 자연스러움을 (음성이 해당 이미지/프롬프트가 묘사하는 공간에서 재생되는 것처럼 들리는가, 소리가 해당 공간에서 울리는 것 같은가) 기준으로 평가해 주세요. Reference와 동일한 condition은 100점을 줘야 하며, 그에 상대적으로 다른 음성들의 점수를 매겨 주세요.<br/>",
