...
DataSender.prototype._buildExportSessionJSON = function(_session) {
  var exportObj = JSON.parse(JSON.stringify(_session));
  if (exportObj && exportObj.trials && exportObj.trials.length) {
    exportObj.trials = exportObj.trials.filter(function(trial) {
      if (!trial || !trial.id) {
        return true;
      }
      return trial.id.indexOf("practice_") !== 0;
    });
  }
...
DataSender.prototype.send = function(_session) {
  var sessionJSON = this._buildExportSessionJSON(_session);
