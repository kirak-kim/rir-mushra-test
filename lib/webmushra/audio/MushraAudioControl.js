...
MushraAudioControl.prototype.initAudio = function() {
  ...
  var sourceChannelCount = this.reference.getAudioBuffer().numberOfChannels;
  var channelCount;
  if (sourceChannelCount === 1 && this.audioContext.destination.channelCount >= 2) {
    channelCount = 2; // duplicate mono to L/R for headphone playback
...
MushraAudioControl.prototype.process = function(audioProcessingEvent) {
  ...
  var sourceChannels = this.reference.getAudioBuffer().numberOfChannels;
  for (channel = 0; channel < outputBuffer.numberOfChannels; ++channel) {
    outputData = outputBuffer.getChannelData(channel);
    // Duplicate mono sources to all output channels (e.g., stereo headphones).
    var inputChannel = (sourceChannels === 1) ? 0 : channel;
    if (inputChannel >= audioBuffer.numberOfChannels) {
      for (sample = 0; sample < outputBuffer.length; ++sample) {
        outputData[sample] = 0;
      }
      continue;
    }
    inputData = audioBuffer.getChannelData(inputChannel);
      ...
      if (fadingActive == 1) { // fade out
        ...
        if (fadingCurrentPosition >= this.audioFadingMaxPosition) {          
          ...
          if (this.audioFadingIn === null) {
            ...
            var fadeInputChannel = (sourceChannels === 1) ? 0 : channel;
            if (fadeInputChannel >= fadeBuffer.numberOfChannels) {
              fadeInputChannel = 0;
            }
            inputData = fadeBuffer.getChannelData(fadeInputChannel);
          }
